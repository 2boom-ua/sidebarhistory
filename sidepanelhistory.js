...
function getMessage(key) {
    ...
    if (!result) {
        var fallback = {
            ...
            'domainFilterTooltip': 'Filter by domain',
            'domainFilterActiveTooltip': 'Remove domain filter',
...
var currentLoadDays = 7;
var maxLoadDays = 67;
var isLoadingMore = false;

// Domain filter state
var currentDomainFilter = null;
...
function getDateGroupKey(timestamp, maxTimestamp) {
    ...
    if (dateOnly.getTime() > monthAgo.getTime()) return 'groupLastMonth';
    return 'groupEarlier';
}
...
function renderHistory(items, listElement) {
    ...
    var groupOrderKeys = ['groupRecent', 'groupToday', 'groupYesterday', 'groupEarlierThisWeek', 'groupLastMonth', 'groupEarlier'];
    ...
    groupOrderKeys.forEach(function(groupKey) {
        if (groups[groupKey] && groups[groupKey].length > 0) {
            ...
            groups[groupKey].forEach(function(item, index) {
                ...
                domainBtn.className = 'action-btn';
                domainBtn.dataset.domain = getHostname(item.url);

                var domainIcon = document.createElement('img');
                domainIcon.className = 'domain-icon';
                var isActive = (currentDomainFilter === getHostname(item.url));
                domainIcon.src = isActive ? 'icons/domain-active.svg' : 'icons/domain.svg';
                domainIcon.alt = 'Filter by domain';
                domainBtn.appendChild(domainIcon);

                var domainTooltip = document.createElement('span');
                domainTooltip.className = 'tooltip';
                domainTooltip.textContent = isActive ? getMessage('domainFilterActiveTooltip') : getMessage('domainFilterTooltip');
                domainBtn.appendChild(domainTooltip);

                if (isActive) {
                    domainBtn.classList.add('active');
                }

                domainBtn.addEventListener('mouseenter', function(e) {
                    e.stopPropagation();
                    domainTooltip.classList.add('show');
                });

                domainBtn.addEventListener('mouseleave', function(e) {
                    e.stopPropagation();
                    domainTooltip.classList.remove('show');
                });
...
function displayHistory(query) {
    ...
    var items = filteredHistoryItems;
    
    if (!currentDomainFilter && !query) {
        items = cachedHistoryItems;
    }
...
// Load more history on scroll to bottom
function loadMoreHistory() {
    if (isLoadingMore || currentLoadDays >= maxLoadDays) return;
    
    isLoadingMore = true;
    var newDays = currentLoadDays + 30;
    var loadingIndicator = document.getElementById('loadingIndicator');
    
    loadingIndicator.style.display = 'block';
    loadingIndicator.textContent = 'Loading more...';
    
    loadHistoryToCache(newDays).then(function() {
        currentLoadDays = newDays;
        isLoadingMore = false;
        loadingIndicator.style.display = 'none';
        
        if (currentDomainFilter) {
            filteredHistoryItems = cachedHistoryItems.filter(function(item) {
                return getHostname(item.url) === currentDomainFilter;
            });
        } else {
            filteredHistoryItems = cachedHistoryItems.slice();
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('history-list').addEventListener('scroll', function(e) {
        var el = e.target;
...
// Noise Reduction - domains with infinite scroll
const noiseDomains = [
    'facebook.com',
    'instagram.com',
    'tiktok.com',
...
function noiseReduction(items) {
    ...
    while (i < items.length) {
        var currentHostname = getHostname(items[i].url);
        var j = i;
        var count = 0;
        
        // Count consecutive same hostname
        while (j < items.length && getHostname(items[j].url) === currentHostname) {
            count++;
            j++;
        }
        
        // Check if domain is in noise list AND count > 3
        var isNoiseDomain = noiseDomains.some(function(domain) {
            return currentHostname.includes(domain) || domain.includes(currentHostname);
        });
        
        if (isNoiseDomain && count > 3) {
